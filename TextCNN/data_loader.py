...
class DataSet(list):
    ...
    def numericalize(self, w2id, c2id):
        # //构建词汇表（字典）统计所有可能出现的字  将字标记成 1 2 3 4
...
class DataBatchIterator(object):
    # //简单获取batch 迭代列表每个元素 列表由batch大小构成的元素集合 统计词频减小词表大小
    ...
    def pad_seq_pair(self, samples):
        ...
        max_Ls = max(Ls)
        sent = [self._pad(item[0], max_Ls, self.vocab.stoi, add_bos=True, add_eos=True) for item in pairs]
    ...
    def __iter__(self):
        ...
        for i in range(self.num_batches):
            ...
            # samples = sorted(
            #    samples, F=lambda x: len(x[0]), reverse=True)
